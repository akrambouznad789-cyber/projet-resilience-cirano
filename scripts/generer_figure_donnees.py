...
def figure_knn_geographique() -> None:
    ...
    # Marge calculée séparément par axe (proportionnelle à sa propre plage) : une
    # marge unique basée sur la plus grande dimension sur-remplissait l'axe le
    # plus court (jusqu'à 42 % de sa plage contre 35 % pour l'autre), ce qui
    # laissait de grandes bandes vides en haut/bas de la figure.
    marge_x = max(400.0, 0.18 * (maxx - minx))
    marge_y = max(400.0, 0.18 * (maxy - miny))
    bbox_local = (minx - marge_x, miny - marge_y, maxx + marge_x, maxy + marge_y)
