...
SORTIE
------
  data/processed/debits_completes_v2.gpkg
...
OUTPUT_FILE = os.path.expanduser(
    "~/projects/projet-resilience-cirano/data/processed/debits_completes_v2.gpkg"
)
OUTPUT_LAYER = "debits_completes_v2"
...
def _extrapoler_extremites(vals: np.ndarray, annees: list[int]) -> np.ndarray:
    ...
    if mask_nan.any():
        ...
        result[mask_nan] = np.maximum(reg.predict(x_nan), 0.0)
...
def mice_cam(gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    cam_imputed  = np.maximum(M_imputed[n_train:, N_ANNEES:], 0.0)
...
def phase2_geographique(gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    for coord, type_cible, idx_seg in zip(coords_cible, types_cibles, idx_cibles):
        ...
        if len(pos_prio) < K_VOISINS:
            pos_fb    = positions[~meme_type][:K_VOISINS - len(pos_prio)]
            dist_fb   = distances[~meme_type][:K_VOISINS - len(pos_prio)]
